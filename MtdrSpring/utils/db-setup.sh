...
CREATE TABLE $U.Team (
 ...
 managerID NUMBER,
 FOREIGN KEY (managerID) REFERENCES $U.telegramUser(id)
);
CREATE TABLE $U.Sprint (
 ...
 endDate DATE,
 teamID NUMBER,
 FOREIGN KEY (teamID) REFERENCES $U.Team(id)
);
CREATE TABLE $U.Task (
 id NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
 description VARCHAR2(4000),
 ...
 userID NUMBER,
 sprintID NUMBER,
 FOREIGN KEY (userID) REFERENCES $U.telegramUser(id),
 FOREIGN KEY (sprintID) REFERENCES $U.Sprint(id)
);
commit;
