...
BEGIN
   EXECUTE IMMEDIATE 'CREATE USER $U IDENTIFIED BY "$DB_PASSWORD" DEFAULT TABLESPACE data QUOTA UNLIMITED ON data';
...
CREATE TABLE TODOUSER.telegramUser (
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      ELSE
         RAISE; -- Re-raise the exception if it's not because the user already exists
      END IF;
END;
