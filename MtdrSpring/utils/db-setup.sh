...
CREATE TABLE $U.Team (
 ...
 managerID NUMBER REFERENCES $U.telegramUser(id)
);
CREATE TABLE $U.Sprint (
 ...
 startDate TIMESTAMP,
 endDate TIMESTAMP,
 teamID NUMBER REFERENCES $U.Team(id)
);
CREATE TABLE $U.Task (
 id NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
 description VARCHAR2(4000),
 status VARCHAR2(255),
 userID NUMBER REFERENCES $U.telegramUser(id),
 sprintID NUMBER REFERENCES $U.Sprint(id)
);
commit;
